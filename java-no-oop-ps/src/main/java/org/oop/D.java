...
public class D {
    ...
    public Comment cc(Comment comment) {
        comment.id = (long) (comments.size() + 1);
        comments.add(comment);
        return comment;
    }

    // Get comments by article ID
    public List<Comment> gc(Long articleId) {
    ...
    public boolean dc(Long commentId) {
        return comments.removeIf(comment -> comment.id.equals(commentId));
    }
